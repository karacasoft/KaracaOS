...
SYS_RET allocate_blocks(ext2_mount_context_t *mnt, uint32_t *allocated_block, size_t count);
SYS_RET allocate_inode(ext2_mount_context_t *mnt, uint32_t *allocated_inode, BOOLEAN is_dir, BOOLEAN allocate_given);
...
SYS_RET ext2_format_device(block_device_t *blockDevice) {
    ...
    if(ret = fill_superblock(&superblock, block_size, blockDevice->sectorSize, blockDevice->size_in_kb, inodes_per_group)) {
        return ret;
    }
    ...
    for (i = 0; i < block_group_count; i++) {
        ...
        if(i == block_group_count - 1) {
            ...
            uint32_t free_blocks = block_group_desc[i].bg_free_blocks_count + inode_table_length + 4;
            ...
            initial_block_bitmap[breaking_byte] = ~initial_block_bitmap[breaking_byte];
            
            for(j = breaking_byte + 1; j < 1024; j++) {
                initial_block_bitmap[j] = 0xFF;
            }
        }
        ext2_write_blocks(blockDevice, curr_block++, (void *)initial_block_bitmap, 1);

        // Write inode bitmap
        ext2_write_blocks(blockDevice, curr_block++, (void *)initial_inode_bitmap, 1);
    ...
    if((ret = ext2_mount_device(blockDevice, &mnt))) {
        kaos_printf("An error occured while mounting newly created system (Code: %d)\n", ret);
        return ret;
    }
    uint32_t inode_nr = 1;
    allocate_inode(&mnt, &inode_nr, FALSE, TRUE);

    for(i = 3; i < 11; i++) {
        allocate_inode(&mnt, &i, FALSE, TRUE);
    }

    uint32_t root_inode_nr = 2;
    allocate_inode(&mnt, &root_inode_nr, TRUE, TRUE); // / directory
    i = 11;
    allocate_inode(&mnt, &i, TRUE, TRUE); // /lost+found directory
    ...
    ext2_add_link(&mnt, 2, ".", 2, 1, 2);
    ext2_add_link(&mnt, 2, "..", 2, 2, 2);

    ext2_add_link(&mnt, 2, "lost+found", 2, 10, 11);
    ext2_add_link(&mnt, 11, ".", 2, 1, 11);
    ext2_add_link(&mnt, 11, "..", 2, 2, 2);
...
SYS_RET ext2_mount_device(block_device_t *block_device, ext2_mount_context_t *mnt) {
    ...
    if((ret = ext2_read_blocks(block_device, 1, superblock_ptr, 1))) {
        kaos_printf("Unexpected error while mounting device, (ext2_read_blocks: ret: %d)\n", ret);
        return ret;
    }
    ...
    if((ret = ext2_write_blocks(block_device, 1, superblock_ptr, 1))) {
        kaos_printf("Unexpected error while mounting device, (ext2_write_blocks: ret: %d)\n", ret);
        return ret;
    }
...
SYS_RET ext2_unmount_device(ext2_mount_context_t *mnt) {
    ...
    if((ret = ext2_write_blocks(block_device, 1, superblock_ptr, 1))) {
        kaos_printf("Unexpected error while unmounting device, (ext2_write_blocks: ret: %d)\n", ret);
        return ret;
    }
...
SYS_RET ext2_add_link(ext2_mount_context_t *mnt, uint32_t dir_inode, const char *name, uint8_t type, uint8_t name_len, uint32_t inode) {
    ...
    uint8_t block_nr = dir.i_blocks / 2;
    uint32_t dir_block_nr = dir.i_block[block_nr];
    ...
    uint16_t rec_len = 8 + name_len + (4 - name_len % 4) % 4;

    void *buffer_pos = element.buffer_pos;

    *((uint32_t *)buffer_pos) = inode;
    *((uint16_t *)buffer_pos + 2) = rec_len;
    ...
    *((uint16_t *)buffer_pos + 2) = 1024 - (rw_buffer - buffer_pos);
...
SYS_RET ext2_read_inode(ext2_mount_context_t *mnt, ext2_inode_t *inode, uint32_t inode_nr) {
    ...
    ext2_read_blocks(mnt->ctx_block_device, desc_table[target_block_group].bg_inode_bitmap,
    ...
    uint32_t target_block = desc_table[target_block_group].bg_inode_table + local_inode_index / 8;
...
SYS_RET ext2_write_inode(ext2_mount_context_t *mnt, ext2_inode_t *inode, uint32_t inode_nr) {
    ...
    ext2_write_blocks(mnt->ctx_block_device, desc_table[target_block_group].bg_inode_bitmap,
        (void *) inode_bitmap_block, 1);
...
SYS_RET ext2_read_blocks(block_device_t *block_device, uint32_t block_id, void *buffer, size_t count) {
    ...
    for(i = 0; i < count * sectors_per_block; i++) {
        if((ret = block_dev_read(block_device, block_id * sectors_per_block + i, (buffer + i * block_device->sectorSize)))) {
...
SYS_RET ext2_write_blocks(block_device_t *block_device, uint32_t block_id, void *buffer, size_t count) {
    ...
    size_t sectors_per_block = 1024 / block_device->sectorSize;
    for(i = 0; i < count * sectors_per_block; i++) {
        if((ret = block_dev_write(block_device, block_id * sectors_per_block + i, (buffer + i * block_device->sectorSize)))) {
...
SYS_RET fill_block_group_descriptor(ext2_block_group_descriptor_t *bg_desc, const ext2_superblock_t *superblock, uint32_t bg_index) {
    ...
    if((bg_index + 1) * blocks_per_group > total_blocks) {
        // this value will overflow if leftover blocks are not enough to hold the inode table
        // FIXME if you will...
        free_blocks_count = total_blocks - (bg_index * blocks_per_group) - inode_table_length - 4 - 1;
    }
...
SYS_RET allocate_blocks(ext2_mount_context_t *mnt, uint32_t *allocated_blocks, size_t count) {
    SYS_RET ret = SYS_RET_NO_ERROR;
    size_t i = 0;
    ...
    for(i = 0; i < 32; i++) {
        ...
        ext2_read_blocks(mnt->ctx_block_device, desc_table[i].bg_block_bitmap, block_bitmap, 1);
        ...
        for(j = 0; j < 1024; j++) {
            ...
            *_allocated_blocks++ = i * mnt->ctx_superblock.s_blocks_per_group + j * 8 + k + 1;
        ...
        if(found_all) {
            for(j = 0; j < count; j++) {
                size_t byte_idx = ((allocated_blocks[j] - 1) % mnt->ctx_superblock.s_blocks_per_group) / 8;
                size_t bit_offset = ((allocated_blocks[j] - 1) % mnt->ctx_superblock.s_blocks_per_group) % 8;
                block_bitmap[byte_idx] = block_bitmap[byte_idx] | (1 << bit_offset);
            }

            ext2_write_blocks(mnt->ctx_block_device, desc_table[i].bg_block_bitmap, block_bitmap, 1);
...
SYS_RET allocate_inode(ext2_mount_context_t *mnt, uint32_t *allocated_inode, BOOLEAN is_dir, BOOLEAN allocate_given) {
    ...
    if(allocate_given) {
        ...
        ext2_read_blocks(mnt->ctx_block_device, desc_table[i].bg_inode_bitmap, inode_bitmap, 1);
        ...
        for(i = 0; i < 32; i++) {
            if(desc_table[i].bg_free_inodes_count != 0) {
                ...
                for(j = 0; j < 1024; j++) {
                    ...
                    *allocated_inode = i * mnt->ctx_superblock.s_inodes_per_group + j * 8 + k + 1;
                    found = TRUE;
                    break;
                }
    ...
    if(found) {
        ...
        ext2_write_blocks(mnt->ctx_block_device, desc_table[i].bg_inode_bitmap, inode_bitmap, 1);
...
BOOLEAN is_inode_allocated(char inode_bitmap[1024], uint32_t inode_nr) {
    return (inode_bitmap[inode_nr / 8] & (1 << (inode_nr % 8))) ?
        TRUE : FALSE;
}
