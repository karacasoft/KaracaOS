...
SYS_RET mm_init(multiboot_info_t *info) {
    ...
    for(i = 0; i < size; i += sizeof(multiboot_memory_map_t)) {
        multiboot_memory_map_t *entry = (multiboot_memory_map_t *) (info->mmap_addr + i);

        if(entry->type == MULTIBOOT_MEMORY_AVAILABLE) {
            kaos_printf("%d: Addr: %x Len: %x\n", j++, (uint32_t) entry->addr, (uint32_t) entry->len);
...
BOOLEAN mm_is_memory_available(uint32_t size, uint32_t *vm_block) {
    ...
    if(mm_available_block_count < count) return FALSE;

    uint32_t free_vm_blocks = 0;
    uint32_t start_vm_block = *vm_block;
    uint32_t start;
    for(start = start_vm_block; start < 98304 * 8; start++) {
        if(mm_vm_alloc_bitmap[start / 8] == 0xFF) continue;
        uint8_t bitmap_piece = mm_vm_alloc_bitmap[start / 8];

        uint8_t i;
        for(i = 0; i < 8; i++) {
            if(bitmap_piece & 0x1) free_vm_blocks = 0;
            else {
                if(free_vm_blocks == 0) start_vm_block = start + i;
                free_vm_blocks++;
            }
...
SYS_RET mm_alloc(void **ret_addr, uint32_t vaddr, uint32_t size, uint32_t flags) {
    SYS_RET ret;
    uint32_t size_in_blocks = size / 4096;
    if(size_in_blocks == 0) size_in_blocks = 1;
    ...
    if(mm_is_memory_available(size, &vm_block)) {
        while(mm_memory_info[mm_mem_info_idx].flags & MM_ALLOC_MEMORY_INFO_FLAGS_PRESENT) {
            mm_mem_info_idx++;
        }
