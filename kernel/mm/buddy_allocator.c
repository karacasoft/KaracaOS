...
size_t tree_area_size = 20 * 1024 * 1024;
...
SYS_RET __buddy_allocator__alloc_node(binary_tree_node_t **out_node) {
  ...
  size_t i = 0;
  while (!found) {
    if (*((uint32_t *)ptr) == FALSE &&
        (*((uint32_t *)ptr + 1) >= sizeof(binary_tree_node_t) +
                                       sizeof(buddy_allocator__alloc_info_t) +
                                       8)) {
      break;
    }
    ptr += *((uint32_t *)ptr + 1);
  ...
  *((uint32_t *)next_block_ptr + 1) = old_size - *((uint32_t *)ptr + 1);
  ...
  new_alloc_info->is_allocated = FALSE;
...
SYS_RET __buddy_allocator__split_node(binary_tree_node_t *node) {
  ...
  info_left->is_allocated = FALSE;
  info_right->is_allocated = FALSE;
...
SYS_RET buddy_allocator__free(void *addr) {
  ...
  while (info->vaddr != addr) {
    if (info->vaddr < addr) {
      node = node->right;
    } else if (info->vaddr > addr) {
      node = node->left;
    }
    if (node == NULL) {
  ...
  void *i;
  for (i = info->vaddr; i < info->vaddr + info->len; i += 4096) {
    mm_free(i);
  }
...
SYS_RET __buddy_allocator__find_space(size_t len, binary_tree_node_t *node,
  ...
  binary_tree_node_t *found_node = NULL;
